...
async fn chat_mode_stream_cli() {
    ...
    let bin = env::var("CARGO_BIN_EXE_codex").expect("CARGO_BIN_EXE_codex should be set");
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    let bin2 = env::var("CARGO_BIN_EXE_codex").expect("CARGO_BIN_EXE_codex should be set");
